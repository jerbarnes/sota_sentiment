...
def test_embeddings(embedding_file, file_type):
    ...
    for name, dataset in zip(names, datasets):
        ...
        for i, c in enumerate(test_cs):
            ...
            best_f1 = 0
            best_c = 0
            dev_classifier = LogisticRegression(C=c)
            h = dev_classifier.fit(dataset._Xtrain, dataset._ytrain)
            labels = sorted(set(dataset._ytrain))
            if len(labels) == 2:
                average = 'binary'
            else:
                average = 'micro'
            dev_pred = dev_classifier.predict(dataset._Xdev)
            dev_f1 = f1_score(dataset._ydev, dev_pred, average=average)
            if dev_f1 > best_f1:
                best_f1 = dev_f1
                best_c = c
        print()
